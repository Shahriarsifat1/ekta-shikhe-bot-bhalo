...
export const ChatInterface = () => {
  ...
  // Enhanced focus management - maintain focus during all states
  useEffect(() => {
    const focusInput = () => {
      if (inputRef.current) {
    ...
    // Focus after any state change
    const timeoutId = setTimeout(focusInput, 100);

    return () => clearTimeout(timeoutId);
  }, [messages, isLoading, isTyping]);

  // Additional focus maintenance
  useEffect(() => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        ...
        setIsLoading(false);
        // Ensure focus is maintained after response
        setTimeout(() => {
          if (inputRef.current) {
            inputRef.current.focus();
          }
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-purple-100 to-pink-50 flex flex-col">
      {/* Chat Messages Area */}
